...
if 'snakemake' in globals():
    ...
    enformer_file = _sn.input.enformer_scores # type: ignore
...
def read_enformer(enf_path):
    print(f"Loading Enformer scores from: {enf_path}")
...
def main():
    enf_df = read_enformer(enformer_file)
    meta_cols = ['chrom', 'pos', 'id', 'ref', 'alt']
    track_cols = [c for c in enf_df.columns if c not in meta_cols]
    ...
    enf_df['join_key'] = enf_df.apply(make_join_key, axis=1)
    mpra_df['join_key'] = mpra_df.apply(make_join_key, axis=1)

    merged = pd.merge(enf_df, mpra_df, on='join_key', how='inner', suffixes=('', '_mpra'))
    if merged.empty:
        print('No overlap between Enformer output and VCF MPRA entries after normalization. Exiting.')
        # Optionally, print a few sample keys from each side for debugging
        print('Sample Enformer keys:', enf_df['join_key'].head(5).tolist())
